...
def annotate_graph_deterministic(graph, levels, X, model, act_np,
    ...
    # Map layer name -> list of (neuron_index, node_id, display_name)
    layer_neuron_names = {"input": [(i, f"input:{i}", n) for i, n in enumerate(input_names)]}
    linear_idx = 0
    for module in model:
        if isinstance(module, nn.Linear):
            ...
            layer_neuron_names[lname] = [
                (ni, f"{lname}:{ni}", graph[f"{lname}:{ni}"]["name_hint"])
    ...
    for lname, prev_lname in layer_order:
        ...
        for ni, nid, _ in layer_neuron_names[lname]:
            ...
            # Use top input correlation: sign + feature name
            if node["input_corr"]:
                top = node["input_corr"][0]
                sign = "+" if top["r"] >= 0 else "-"
                # shorten feature name to 12 chars for readability
                fname = top["name"][:14].replace(" ", "_")
...
def ollama_name(nid, node, graph):
    import urllib.request, json as _json, re, http.client, urllib.parse
