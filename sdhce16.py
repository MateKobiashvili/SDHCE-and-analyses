...
def symbolic_name(nid, node, graph, imap, scaler, input_names,
    ...
    for feat, net_w in net_contribs.items():
        ...
        # use actual interval edges if available
        if feat in imap:
            edges, labels = imap[feat]
            # net_w is a path weight, not a raw feature value
            # use sign + magnitude percentile among survivors to pick bin
            if net_w > 0:
                bin_idx = min(int(pct / (100 / len(labels))), len(labels) - 1)
            else:
                bin_idx = max(len(labels) - 1 - int(pct / (100 / len(labels))), 0)
            lbl = labels[bin_idx]
        else:
            lbl = interval_labels[-1] if net_w > 0 else interval_labels[0]
